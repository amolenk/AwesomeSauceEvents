...
export async function GET(
    ...
    try {
        ...
        const registration = await admittoRequest<PartnerRegistrationDetailDto>(
            eventPath(eventSlug, `/registrations/${encodeURIComponent(registrationId)}`)
        );
...
export async function PUT(
    ...
    try {
        ...
        await admittoRequest<void>(eventPath(eventSlug, `/registrations/${encodeURIComponent(registrationId)}`), {
            method: "PUT",
            body: {
