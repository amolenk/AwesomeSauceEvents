...
export interface PartnerTicketedEventDetailsDto {
    name: string;
...
export function eventPath(eventSlug: string, path: string): string {
    return `/api/events/${encodeURIComponent(eventSlug)}${path}`;
}

// The event slug is always provided by the caller through the `event` query
// string parameter. When it is missing the site has nothing to show, so the
// caller should treat a null result as a 404.
